...
Paperclip::Attachment.default_options[:s3_host_alias] = 's3-eu-west-1.amazonaws.com'
#:s3_host_alias => "my.aws.alias"

#Paperclip.interpolates(:s3_sg_url) do |att, style|
Paperclip.interpolates(:s3_alias_url) do |att, style|
  #"#{att.s3_protocol}://#{Paperclip::Attachment.default_options[:default_url]}/#{att.bucket_name}/#{att.path(style)}"
  "#{att.s3_protocol}://#{Paperclip::Attachment.default_options[:s3_host_alias]}/#{att.bucket_name}/#{att.path(style)}"
end